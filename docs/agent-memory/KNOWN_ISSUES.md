...
| K-013 | Hours | Short-hours fines land the moment a shift closes, with no grace period and no admin "relax" button (unlike absences/hourly slots). The only waiver path is approving an *Early Sign-off* or *Extended Break* request for that date. |
