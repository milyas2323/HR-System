...
| K-012 | Hours | Worked hours are derived from `start_time`/`end_time` only — an employee who leaves early but closes the shift late still shows full hours. No break start/stop tracking exists (the 1h deduction is flat by policy). |
| K-013 | Hours | Short-hours fines land the moment a shift closes, with **no grace period** — a shift 20 seconds under 8h is fined the full PKR 1,000. Waiver paths: admin **Waive short hours** button (Reports → daily breakdown), or an approved *Early Sign-off* / *Extended Break* request for that date. Add a grace constant if the seconds-level strictness proves noisy. |
